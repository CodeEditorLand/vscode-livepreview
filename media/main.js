...
(function () {
	...
	function handleMessage(message) {
		switch (message.command) {
			...
			case "changed-history": {
				...
				if (msgJSON.element) {
					document.getElementById(msgJSON.element).disabled =
						msgJSON.disabled;
				}
				adjustTabIndex();
				break;
	...
	function fadeElement(appear, elem) {
		...
		fadeLinkID = setInterval(function () {
			if (elem.style.opacity == finalOpacity) {
				...
				elem.style.opacity =
					parseFloat(elem.style.opacity) +
					parseFloat(appear ? 0.1 : -0.1);
			}
		}, 25);
	}
	...
	function addNavButtonListeners() {
		...
		for (const menuNavItem of menuNavGroup) {
			menuNavItem.addEventListener("mouseover", () =>
				menuNavItem.focus(),
			);
		}
